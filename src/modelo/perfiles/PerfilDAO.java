...
public class PerfilDAO {
    ...
    //Obtener perfiles por nombre
    // Obtener un perfil por nombre
    public Perfil obtenerPerfilPorNombre(String nombre) {
        BasicDBObject query = new BasicDBObject();
        query.put("NOMBRE", nombre);

        DBObject doc = conexion.getColeccionPerfiles().findOne(query); // Acceder a la colección de perfiles
        if (doc != null) {
            String posicion = (String) doc.get("POSICION");
            Float altura = ((Double) doc.get("ALTURA")).floatValue();
            Float peso = ((Double) doc.get("PESO")).floatValue();
    ...
    public boolean eliminarPerfil(String nombre) {
        ...
        conexion.getColeccionPerfiles().remove(query); // Acceder a la colección de perfiles
        return true;
    }
}